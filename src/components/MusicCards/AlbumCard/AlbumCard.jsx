...
import { Heart, Music, Share } from 'iconsax-reactjs';
  ...
  return (
    ...
    >
      <div className="flex items-center lg:p-3">
        ...
        >
          <img
            className="z-1 size-21.25 min-h-21.25 min-w-21.25 cursor-pointer rounded-sm object-cover transition-all group-hover:opacity-50 lg:group-hover:opacity-100"
            src={cover ?? noCoverImg}
            alt={title}
            loading="lazy"
          />
          <div
            className={`absolute z-2 flex size-[70%] items-center justify-center rounded-full border border-white bg-cover bg-center bg-no-repeat opacity-0 transition-all duration-300 group-hover:opacity-100 lg:left-12 lg:z-auto lg:size-20 lg:border-white/60 lg:opacity-60 ${isCurrentAlbumPlaying ? 'animate-infinite-rotate' : 'group-hover:animate-infinite-rotate'}`}
            style={{
          ...
          >
            <span className="size-[31%] rounded-full border border-white lg:size-5.25 lg:border"></span>
          </div>
        </Link>
